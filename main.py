...
def get_image():
    ...
    png_files = [file for file in os.listdir(absolute_path) if file.endswith('.png') or file.endswith('.jpg') or file.endswith('.jpeg')]   # recuperando todos os arquivos imagens da pasta conjunto-de-dados
...
def generate_possible_plates(plate_text, max_substitutions=2):
    """Gera possíveis placas considerando erros comuns nas leituras"""
    substitutions = {
        '2': ['Z'],
        'Z': ['2'],
        '1': ['I'],
...
def is_plate(image, contour):
    ...
    # Ignorando retangulos inválidos para placas
